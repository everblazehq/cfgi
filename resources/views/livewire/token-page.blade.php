<div class="min-h-screen bg-bg-primary text-text-primary p-2 md:p-8 overflow-x-hidden">
    ...
    <div class="space-y-20">
        ...
        <div>
            <x-token-page.fear-greed-index :cfgData="$cfgData" :coin="$coin" wire:model="period" :countdownPercentage="$countdownPercentage"
                :timeRemaining="$timeRemaining" />
        </div>
        {{-- Sandbox Alerts Section --}}
        <x-token-page.sandbox-alerts :configured-alerts="0" :remaining-alerts="100" />
            ...
            @if (!empty($cfgData) && isset($cfgData[0]))
                @foreach ($cfgData[0] as $key => $score)
                    @if (!in_array($key, ['date', 'cfgi', 'time']))
                        ...
                        <x-token-page.sentiment-analysis.card title="{{ $key }} sentiment" :progress="$countdownPercentage"
                            :scoreTimeData="$scoreTimeData" :index="$key" :isBlurred="false" :wire:key="$key" />
                    @endif
                @endforeach
            @else
        ...
        <x-token-page.explanation :indicators="[
            'Main Indicator',
            'Price Score',
            'Volatility',
            'Volume',
