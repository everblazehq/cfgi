...
<div class="flex flex-col bg-bg-primary border border-border-light rounded-xl p-7 gap-10 w-full">
    <div class="flex flex-col gap-5">
        <div class="flex justify-between items-center">
            <h4 class="text-[30px] font-medium">{{ $title }}</h4>
            <x-icons.info tooltip="Sentiment analysis explanation" />
        </div>
        <!--progress bar goes here -->
        <div class="w-full">
            <x-token-page.sentiment-analysis.progress-bar-card :value="$scoreTimeData[0]['score'] ?? 0" />
        </div>
        <div class="flex justify-center">
            <x-global.badges.status-badge value="{{ $scoreTimeData[0]['score'] }}" type="detailed" size="sm"
                indicator="{{ explode(' ', $title)[0] }}" />
        </div>
    </div>
    <div class="flex flex-col gap-5 relative">
        <div class="{{ $isBlurred ? 'blurred' : '' }}">
            @if (is_array($scoreTimeData) && !empty($scoreTimeData))
                <livewire:charts :data="$scoreTimeData" dataType="scoreTimeData" :chartData="[
